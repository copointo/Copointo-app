...
- **Server-side user mirror (super-admin visibility)**: The mobile app's `register()` now calls `POST /api/users/register { id, username, phone, joinedAt }` (after the username claim) so every signed-up player appears in the super-admin's "المستخدمون" page (`UsersPage.tsx`) — not only those who placed an order. The endpoint validates phone uniqueness across different ids (returns 409 with Arabic error), is idempotent on `id` (refreshes username/phone in place but preserves `level`, `totalOrders`, ban flags, etc.), and is also called fire-and-forget on app startup (`loadData`) and after `login()` to backfill existing accounts that registered before this change. `syncUserToServer()` helper in `AppContext.tsx` wraps the call.
