...
- **Cafe Purchase History** (`app/cafe/[id]/history.tsx`): a small 🛍️ cart badge at the top-left of each big "اطلب قهوة" / "احجز طاولة" action card on the cafe landing page (`cafe/[id].tsx`) opens this screen. It shows: the current cart filtered to this cafe (with "إتمام الشراء" CTA → `/cart`), an active-order banner if one is running, past orders for this cafe (status pill, items, type/table/payment, total), and past table bookings (status pill, table+guests+hours, date+time, price). Both lists are filtered to the **last 30 days** client-side with a notice banner explaining the retention. Data comes from `/api/orders?phone=&cafeId=` (NEW endpoint in `routes/index.ts`, joins `cafeName` server-side) and the existing `/api/bookings?phone=`. Refreshes on `useFocusEffect`.
