...
- **Web popup blocked.** `window.open(checkoutUrl)` ran AFTER `await createPaymentSession` → no longer in the click gesture → silently blocked. Fix: open a placeholder `about:blank` tab synchronously inside the click, then set `webWin.location.href = checkoutUrl` once ready (fallback to plain open if null). Native uses the in-app WebView modal, unaffected.
