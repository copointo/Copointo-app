...
- **Web popup blocked.** `window.open(checkoutUrl)` ran AFTER `await createPaymentSession` → no longer in the click gesture → silently blocked. The synchronous `about:blank` placeholder-tab trick ALSO proved unreliable inside the Replit preview iframe. **Final fix:** don't auto-open on web at all. After the session is created show a small modal ("إتمام عملية الدفع" + "ادفع الآن" button); the button's `onPress` calls `window.open` from a FRESH user tap (reliably allowed), then starts polling, and falls back to `window.location.href = url` (same tab) if the new tab is still null. A `preparing` overlay covers the slow (~40s) session-create wait. **Why:** programmatic popups not tied to a direct click are blocked by browsers and the sandboxed preview iframe; a real button tap is the only dependable trigger. Native uses the in-app WebView modal, unaffected.
