...
function FlyingCreature({ c, glow, emoji, dir, gap, spin = false, trail, steady = false }: { c: Creature; glow: string; emoji: string; dir: FlyDir; gap: number; spin?: boolean; trail?: string; steady?: boolean }) {
  ...
  let transform: any[] = [];
  const spinRot = spin
    ? fly.interpolate({ inputRange: [0, 1], outputRange: ["0deg", "720deg"] })
    : null;
  ...
  if (dir === "horizontal") {
    ...
    if (spin && spinRot) {
      transform = [{ translateX: tx }, { translateY: ty }, { rotate: spinRot }];
    } else {
    ...
    transform = spin && spinRot
      ? [{ translateY: ty }, { translateX: tx }, { rotate: spinRot }, { scale: sc }]
      : [{ translateY: ty }, { translateX: tx }, { scale: sc }];
  } else {
    ...
    >
      {trail && (
        <Text style={{
          fontSize: c.size * 0.8,
          marginRight: -4,
          textShadowColor: glow, textShadowRadius: 10,
          opacity: 0.85,
        }}>{trail}</Text>
      )}
      <Text style={{
  ...
  return (
    <View style={[
      ...
      {eff === "flames" && (
        <>
          <GlowBurst color={highlight} borderRadius={borderRadius} />
          <FlyingCreatures glow={highlight} borderRadius={borderRadius} emoji="🔥" dir="fall" count={8} gap={1500} steady sizeMin={18} sizeMax={32} />
          <View pointerEvents="none" style={[StyleSheet.absoluteFill, { borderRadius, overflow: "hidden" }]}>
            <Sparkles color={highlight} count={14} sizeMin={1.5} sizeMax={3} />
          </View>
        </>
      ...
      {eff === "water" && (
        <>
          <GlowBurst color={highlight} borderRadius={borderRadius} />
          <FlyingCreatures glow={highlight} borderRadius={borderRadius} emoji="🌧️" dir="fall" count={8} gap={1500} steady sizeMin={18} sizeMax={32} />
          <View pointerEvents="none" style={[StyleSheet.absoluteFill, { borderRadius, overflow: "hidden" }]}>
            <Sparkles color={highlight} count={14} sizeMin={1.5} sizeMax={3} />
          </View>
        </>
