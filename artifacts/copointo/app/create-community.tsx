...
export default function CreateCommunityScreen() {
  ...
  const toggle = (id: string) => {
    ...
    setSelected(prev => {
      ...
      if (next.has(id)) {
        ...
        // +1 because the creator counts toward the cap
        if (next.size + 1 >= COMMUNITY_MAX_MEMBERS) {
          setErr(`الحد الأقصى ${COMMUNITY_MAX_MEMBERS} عضواً`);
          return prev;
        }
        next.add(id);
      }
  ...
  const submit = async () => {
    ...
    setBusy(true);
    try {
      const r = await createCommunity(name.trim(), Array.from(selected), avatar ?? undefined);
      if (!r.ok) {
        setErr(r.error);
        setBusy(false);
        return;
      ...
      router.replace(`/community-info?id=${r.community.id}`);
    } catch (_) {
      setErr("تعذّر إنشاء المجتمع");
      setBusy(false);
    }
  };
  ...
  return (
    <View style={[styles.container, { paddingTop: topPad }]}>
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>🏛️ إنشاء مجتمع</Text>
      </View>
      ...
      >
        ...
        <View style={styles.avatarSection}>
          ...
          <Text style={styles.avatarHint}>صورة المجتمع (اختياري)</Text>
        </View>

        {/* Name */}
        <Text style={styles.label}>اسم المجتمع</Text>
        <TextInput
          style={styles.input}
          placeholder="مثلاً: عشّاق الإسبريسو"
        ...
        <View style={styles.infoBox}>
          ...
          <Text style={styles.infoText}>
            المجتمع يحتاج 2-{COMMUNITY_MAX_MEMBERS} أعضاء. ستُرسل دعوة لكل من تختاره.
          </Text>
        </View>

        {/* Members */}
        <View style={styles.membersHeader}>
          <Text style={styles.label}>اختر الأعضاء (دعوة)</Text>
        ...
        {friendList.length === 0 ? (
          <View style={styles.emptyWrap}>
            ...
            <Text style={styles.emptyText}>
              ليس لديك أصدقاء بعد. أضف أصدقاء أولاً ثم أنشئ مجتمعاً.
            </Text>
          </View>
        ) : (
          friendList.map(f => {
              ...
              >
                ...
                <View style={{ flex: 1 }}>
                  ...
                  <Text style={styles.rowSub}>
                    @{f.gameUsername} · ☕ {f.totalOrders ?? 0}
                  </Text>
                </View>
